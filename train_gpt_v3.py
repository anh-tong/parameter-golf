...
---
Modifications vs baseline (train_gpt.py):

SHARED-WEIGHT INTERLEAVED BLOCKS WITH PER-LAYER OUTPUT GATES
=============================================================
All 9 transformer depths share just TWO sets of weight matrices:
  - one CausalSelfAttention + MLP for the encoder half (layers 0..3)
  - one CausalSelfAttention + MLP for the decoder half  (layers 4..8)

Within each shared block, each layer steers the computation via learned
per-layer OUTPUT GATE VECTORS:

    gated_output = linear_output * sigmoid(gate[layer_idx])

where gate[layer_idx] is a vector of shape [out_dim], one row per layer,
stored as a simple nn.Parameter of shape [num_layers, out_dim].

This is applied after every linear projection (Q, K, V, proj, fc, mlp_proj).
The gates learn which output dimensions to emphasize or suppress at each depth,
effectively giving each layer a different "view" through the shared weights.

Parameter budget:
  - Shared weights: same size as one baseline block (saves ~8x on matrix params)
  - Gate overhead: num_layers * (dim + kv_dim + kv_dim + dim + mlp_hidden + dim)
                   ≈ 9 * 512 * 6 ≈ 27K extra scalars — negligible
  - Net result: ~8-9x fewer matrix parameters in the block portion, offset by
    more iterations being feasible within the size budget

Compile note:
  ParameterList[i] where i is a loop variable in an unrolled for-loop is fully
  supported by torch.compile(fullgraph=True, dynamic=False) — the tracer sees
  each i as a compile-time constant from the unrolled range().
"""
...
from torch import Tensor, nn, threshold
from torch.nn.parallel import DistributedDataParallel as DDP

# -----------------------------
# HYPERPARAMETERS
# -----------------------------
# Shared-weight interleaved run:
# - 9 transformer depths, split into encoder (0..3) and decoder (4..8) halves
# - Each half has ONE shared CausalSelfAttention + ONE shared MLP
# - Per-layer output gate vectors steer each depth through the shared weights
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "gate_q,gate_k,gate_v,gate_proj,gate_fc,gate_mlp_proj",
    ).split(",")
    if pattern
)
...
class SharedCausalSelfAttention(nn.Module):
    """One set of Q/K/V/proj weight matrices shared across all depths that use this block.

    Per-layer steering is passed in as pre-computed gate tensors (already sigmoid-ed),
    so this module contains only the shared parameters and no layer-index logic.
    """
    ...
    def forward(
        self,
        x: Tensor,
        gate_q: Tensor,    # [dim]    — output gate on Q projection
        gate_k: Tensor,    # [kv_dim] — output gate on K projection
        ...
        q = self.c_q(x) * gate_q[None, None, :]    # [B, T, dim]
        k = self.c_k(x) * gate_k[None, None, :]    # [B, T, kv_dim]
        v = self.c_v(x) * gate_v[None, None, :]    # [B, T, kv_dim]
        ...
        cos, sin = self.rotary(seqlen, x.device, dtype)
...
class SharedMLP(nn.Module):
    ...
    def __init__(self, dim: int, mlp_mult: int):
        ...
        self.fc   = CastedLinear(dim,        self.hidden, bias=False)
        self.proj = CastedLinear(self.hidden, dim,        bias=False)
        self.proj._zero_init = True

    def forward(self, x: Tensor, gate_fc: Tensor) -> Tensor:
        h = torch.relu(self.fc(x) )   # [B, T, hidden]
        return self.proj(h.square()* gate_fc[None, None, :])
...
class SharedBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn = SharedCausalSelfAttention(dim, num_heads, num_kv_heads, rope_base)
        self.mlp  = SharedMLP(dim, mlp_mult)

    def forward(
        self,
        x: Tensor,
        x0: Tensor,
        resid_mix: Tensor,   # [2, dim]
        attn_scale: Tensor,  # [dim]     ← FIX: was passing mlp_scale twice
        mlp_scale: Tensor,   # [dim]
        gate_q: Tensor,      # [dim]
        gate_k: Tensor,      # [kv_dim]
        gate_v: Tensor,      # [kv_dim]
...
class GPT(nn.Module):
    def __init__(
        ...
        self.topk_sharpness = topk_sharpness
        self.topk_percentage = topk_percentage
        self.num_shared_blocks = num_shared_blocks
        self.skip_weights = nn.Parameter(torch.ones(self.num_skip_weights, model_dim, dtype=torch.float32))

        # Shared blocks — weights reused across all layers (round-robin by block_id)
    ...
    def soft_topk(self, gate_logits: Tensor) -> Tensor:
        """Differentiable soft top-k gate.

        Maps raw gate logits of shape [..., d] to soft binary masks in (0, 1).
        At init (logits=0), exactly topk_percentage fraction of channels are active.

        Strategy: threshold = (1 - topk_percentage) empirical quantile of the
        logits (detached), then sigmoid(sharpness * (logits - threshold)).
        Gradients flow through sigmoid only — the threshold is a straight-through
        constant — so training is stable and the percentage stays calibrated.
        """
        with torch.no_grad():
...
def main() -> None:
    ...
    n_gates   = sum(
        p.numel() for name, p in block_named_params
        if any(g in name for g in ("gate_q", "gate_k", "gate_v", "gate_proj", "gate_mlp_proj"))
    )
    log0(f"model_params:{n_params} (shared_matrices:{n_matrix} gate_vectors:{n_gates})")
